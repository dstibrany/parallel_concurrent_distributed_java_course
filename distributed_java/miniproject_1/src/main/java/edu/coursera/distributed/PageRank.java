    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
            ...
            return sites
                ...
                .flatMapToPair(tuple -> {
                    ...
                    Website src = tuple._1();
                    Double rank = tuple._2();
                    int numEdges = src.getNEdges();
                    Iterator<Integer> edgeIterator = src.edgeIterator();
                    while (edgeIterator.hasNext()) {
                        contribs.add(new Tuple2<>(edgeIterator.next(), rank / (double) numEdges));
                    }
                    return contribs;
                })
                .reduceByKey((accum, n) -> {
